...
function maybeScheduleBot(room) {
  ...
  if (room.botTimerFor === cur.id) return;
  room.botTimerFor = cur.id;
  const delay = BOT_MIN_DELAY_MS + Math.random() * (BOT_MAX_DELAY_MS - BOT_MIN_DELAY_MS);
  setTimeout(() => {
    if (room.botTimerFor !== cur.id) return;
    room.botTimerFor = null;
    if (room.phase !== 'playing') return;
    const stillCur = room.players[room.turnIndex];
    if (!stillCur || stillCur.id !== cur.id) return;
    const cards = chooseBotMove(room, stillCur);
    processTurn(room, stillCur.id, cards);
  }, delay);
}

function afterStateChange(room) {
  broadcastRoom(room);
...
function scheduleCleanup(room) {
  ...
  room.cleanupTimer = setTimeout(() => {
    const stillEmpty = room.players.every((p) => p.isBot || !p.connected);
    if (stillEmpty) {
...
io.on('connection', (socket) => {
  ...
  socket.on('start_game', () => {
    ...
    if (room.players.length < 3) {
      socket.emit('error_msg', 'Нужно минимум 3 игрока (можно добавить ботов)');
      return;
    }
    startRound(room);
    afterStateChange(room);
  ...
  socket.on('play_card', ({ cards }) => {
    ...
    processTurn(room, socket.id, cards);
  });
  ...
  socket.on('disconnect', () => {
    ...
    broadcastRoom(room);
    const anyoneLeft = room.players.some((p) => p.isBot || p.connected);
